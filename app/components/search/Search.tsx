...
import { Ref, RefObject, Suspense, SyntheticEvent, useEffect, useRef, useState } from "react";
...
const Search = ({ onlyInput = false }: SearchProps) => {
    ...
    //         const element = courseResultsRef.current?.children[courseActiveIndex] as HTMLLIElement | undefined;
    //         if (e.key === "ArrowDown") {
    //             e.preventDefault();
    //             const next = Math.min(courseActiveIndex + 1, courseResultsRef.current?.childNodes?.length! - 1);
    //             setCourseActiveIndex(next);
    //             (courseResultsRef.current?.children[next] as HTMLLIElement).focus();
    //             (courseResultsRef.current?.children[next - 1]?.firstChild as HTMLLIElement).classList.remove("opacity-75");
    //             (courseResultsRef.current?.children[next]?.firstChild as HTMLLIElement).classList.add("opacity-75");
    //         } else if (e.key === "ArrowUp") {
    //             e.preventDefault();
    //             const prev = Math.max(courseActiveIndex - 1, 0);
    //             setCourseActiveIndex(prev);
    //             (courseResultsRef.current?.children[prev] as HTMLLIElement).focus();
    //             (courseResultsRef.current?.children[prev + 1]?.firstChild as HTMLLIElement).classList.remove("opacity-75");
    //             (courseResultsRef.current?.children[prev]?.firstChild as HTMLLIElement).classList.add("opacity-75");
    //         } else if (e.key == "Enter") {
    ...
    // }, [courseActiveIndex, selectedInstructor]);
    ...
    useEffect(() => {
        ...
        // this is true for elements that navigate by directly clicking on it
        // and false if you need to click on an inner link
        const clickElem = isSearchingByInstructor ? selectedInstructor === "" : selectedSubjectArea === "";

        const handler = (e: KeyboardEvent) => {
            const element = ref.current?.children[activeIndex] as HTMLLIElement | undefined;

            if (e.key === "ArrowDown") {
                e.preventDefault();
                const next = Math.min(activeIndex + 1, ref.current?.childNodes?.length! - 1);
                setActiveIndex(next);
                (ref.current?.children[next] as HTMLLIElement).focus();
                // opacity-75 to distinguish between hover?
                (ref.current?.children[next - 1]?.firstChild as HTMLLIElement).classList.remove("opacity-75");
                (ref.current?.children[next]?.firstChild as HTMLLIElement).classList.add("opacity-75");
            } else if (e.key === "ArrowUp") {
                e.preventDefault();
                const prev = Math.max(activeIndex - 1, 0);
                setActiveIndex(prev);
                (ref.current?.children[prev] as HTMLLIElement).focus();
                (ref.current?.children[prev + 1]?.firstChild as HTMLLIElement).classList.remove("opacity-75");
                (ref.current?.children[prev]?.firstChild as HTMLLIElement).classList.add("opacity-75");
            } else if (e.key === "Enter") {
                if (clickElem) {
                    element?.click();
                } else {
    ...
    return (
        <div className="flex flex-col align-center justify-center w-full">
            {!onlyInput && <h1 className="text-2xl lg:text-3xl mb-6 text-center font-bold">{prompt}</h1>}
            ...
            <div className="mt-6">
                {isSearchingByInstructor && instructorQuery !== "" && selectedInstructor === "" && (
                ...
                {isSearchingByInstructor && selectedInstructor && (
                    <CourseQueryResults
                        courses={Object.keys(instructors[selectedInstructor])
                            .map((subjectArea) => {
                                return Object.values(instructors[selectedInstructor][subjectArea]);
                            })
                            .flat()}
                        query=""
                ...
                {!isSearchingByInstructor &&
                    (subjectAreaQuery !== "" && selectedSubjectArea === "" ? (
                        ...
                        !onlyInput &&
                        subjectAreaQuery === "" && <h1 className="text-2xl lg:text-3xl text-center font-bold">department</h1>
                    ))}
                {selectedSubjectArea && (
            ...
            <div className="flex items-center justify-center mt-8">
                ...
                >
                    Search by {isSearchingByInstructor ? "department" : "professor"} instead
                </span>
            </div>
        </div>
    );
