...
export function matchCourse(query: string) {
    ...
    return (row: ReturnType<typeof getSubjectAreaCourses>[number]) => {
        ...
        const lastCharCode = catalogNumber.toLowerCase().charCodeAt(catalogNumber.length - 1);
...
const CatalogNumberQueryResults = ({ courses, subjectArea, query }: CatalogNumberQueryResultsProps) => {
    const subjectAreaCourses = getSubjectAreaCourses(courses, subjectArea);

    return <CourseQueryResults courses={subjectAreaCourses} query={query} matcher={matchCourse} />;
};
