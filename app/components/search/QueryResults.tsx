            ...
            <div className="shadow-lg">
                <ul ref={ref}>
                    {results.map((datum, index) => (
                        <li key={keyExtractor(datum)} onClick={() => onSelectResult(datum)} className="bg-uclaBlue">
                            <div className={`hover:opacity-50 ${index === 0 ? "opacity-75" : ""}`}>
                                {renderResult(datum)}
                            </div>
                        </li>
                    ))}
                    {query !== "" && results.length === 0 && (
                        <li className="text-black bg-white p-4 border-t-gray-100 border-t-2">
                            <div className="flex justify-between">
                                <p className="italic font-normal text-gray-400">{noResultsMessage}</p>
                                <a
